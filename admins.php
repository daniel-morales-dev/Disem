...
if(isset($_POST["btn1"])){
	...
	if($btn=="Buscar"){
		...
		$sql="select * from admin where id='$bus'";
		...
		$sql="select count(id),Max(id) from admin";
		$cs=mysql_query($sql,$cn);
		while($resul=mysql_fetch_array($cs)){
			$count=$resul[0];
			$max=$resul[1];
		...
		$id=$_POST["id"];
		$sql="insert into admin values ('$usuario','$password','$email','$rpassword','$id')";
		...
		$sql="update admin set usuario='$usuario',password='$password',email='$email',rpassword='$rpassword' where id='$id'";
...
if(isset($_POST["btn1"])){
	...
	if($btn=="Listar"){
		...
		$cs=mysql_query($sql,$cn);
		echo "<script>location.href='mostrara.php'</script>";
		echo"<center>
<table border='3'>
<tr>
<td>Usuario</td>
